...
| Agent Gateway — agent-callable signing | ✅ Code-complete + merged: Rust security core (biscuit tokens + `decide` policy + ledger + harness), on-device core (SQLCipher ledger, StrongBox biscuit key, token service, `AgentActionDispatcher`), on-chain CEMP-PQ messaging (B-CEMP), Ktor tailnet transport + token UI + biometric approval (B2), keyless Python relay + persistent wake (C). ✅ On-chain verified on Pudge (2026-06-20): agent AllowAuto 70 CKB `0xf1ac39b1…`, replay/approval/cap denies, CEMP profile `0x1571bfc0…` + encrypted message `0xed8c40a7…` (decrypt round-trip OK). ✅ Relay deployed to wyltek-10700 (systemd `agent-relay.service`, tailnet-bound `http://100.118.188.57:9992` (wyltek-10700.tail6db685.ts.net), 45 pytest on-box, pair/intent/deny verified end-to-end over the tailnet). 🔄 Still pending: on-device instrumented tests; relay HTTPS via `tailscale cert` (or a cleartext exception) for the Android client; optional FCM. Fast-follow: relay-approval post-back, token-gate `/v1/account`. Design + plans in `docs/superpowers/` |
...
* ✅ **sUDT sends from a PQ sub-account fixed + verified on-chain (2026-06-20, tx `0xe296586c…`)** — spends an sUDT cell *from* the ML-DSA-65 lock; conservation confirmed (40000 in → 15000 to classic + 25000 PQ change), committed. **Bug found on first broadcast (lock error 46 = `SignatureVerifyFailed`):** the CKB fee-input selector (`get_cells`) skipped type-script cells but **not data-bearing cells**, so a CEMP notification cell (36 B `MessagePointer` data, no type) on the PQ lock was picked as a fee input. The ML-DSA CighashAll digest reconstruction (`signing.rs::cighash_all_digest`) encodes every CKB fee input as type-less + empty-data, while the on-chain `streamer.rs` hashes the cell's real data → digest divergence → verify fail. (CEMP only ever spent a single type-less input, so this path was never exercised until now.) **Fix:** `get_cells` now filters at the indexer to pure-CKB cells only (`script_len_range`+`output_data_len_range` = `[0x0,0x1]`), matching the digest's assumption and avoiding destroying message/notification cells as fee inputs. ✅ **App parity closed (2026-06-22):** audit found the app's `sendToken` already filtered fee inputs to pure-CKB, but **`sendCkb`, `createProfileCell`, and `sendCempMessage` did not** — each selected inputs by `sortedBy capacity` (no filter) *and* handed the signer a hardcoded type-less/empty-data `MldsaInputCell`, so any PQ send that picked an sUDT or CEMP cell (now present on the smoke-test lock, and picked *first* since they're small) would have hit the same lock error 46. (It passed on 2026-06-14 only because the PQ lock then held pure change cells.) Fix: extracted the proven predicate into `data/CellSelection.isPureCkb(hasType, data)` (7 host-JVM unit tests) and routed all four `MldsaInputCell`-building paths through it. Confirmed via grep that `MldsaInputCell` is built in **only** those four functions (DAO + marketplace-buy are secp-only builders → immune). `:app:compileDebugKotlin` + 26 unit tests green. **On-chain proven (testnet, 2026-06-22)** via the shared `rust-core` signing path (`pq_testnet` harness `spend-pq-appfilter`/`spend-pq-nofilter`, replicating `sendCkb` over the smoke-test PQ lock holding sUDT + CEMP + pure cells): pre-fix (no filter) → `Inputs[0].Lock` error 46 (`0x02e2bb06…`, rejected); app-filter (`isPureCkb`) → pool-accepted `0x394a72e7ab65e8f1eb97d6f7c4f9a39ab2ea1fabc3fd23c01e848ac7be47d16e` (skipped all 4 data/type cells, spent only the pure cell). PR #5. ⏳ Remaining: device test of the in-app PQ send UI (logic now proven on-chain).
