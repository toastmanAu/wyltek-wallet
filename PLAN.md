...
* ⏳ **DAO unlock (phase 2)** remains a UI stub — when built it needs bespoke witness handling (`witnesses[0].input_type` = deposit header index + `since` = withdraw epoch + the withdraw block header dep). `sendToken` (sUDT) migrated but not independently broadcast (needs minting); same proven signer/shape as the verified DAO deposit.
