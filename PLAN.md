...
* ✅ **DAO + sUDT secp paths migrated (2026-06-15)** — `depositDao`, `withdrawDaoPhase1`, and `sendToken` now use the verified `sign_ckb_secp256k1_witness` (single secp group). DAO deposit verified on-chain (tx `0x4274e062…`), which also confirmed a **wrong testnet DAO cell-dep** (`0x8e4966b8…` doesn't exist on Pudge; corrected to genesis `0x8f8c79eb…@2`). DAO unlock (phase 2) remains a UI stub — when built it needs bespoke witness handling (`witnesses[0].input_type` = deposit header index + `since` = withdraw epoch).
