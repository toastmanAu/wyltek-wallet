...
* ⏳ **Other secp callers still on the old path** — DAO deposit/withdraw and sUDT `sendToken` still use the old bare-witness `sign_ckb_secp256k1` (the dep-group fix helps them, but their witness construction is unverified). DAO-withdraw also carries an `input_type` witness field so it needs bespoke handling, not the single-group helper.
